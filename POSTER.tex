...
\block{Nucleation interface: Taking place at the critical dendritic interface}
{
	\begin{minipage}{0.43\textwidth}
		\begin{mdframed}
			...
			\end{align*}
			Governing partial differential equation (PDE) of deformation takes the form
			\begin{align*}
				\partial_{t}\bm{u}
				+
				\nabla \cdot
				...
				= -\rho\nabla V_{e},
			\end{align*}
			where $V_{e}: \mathbb{R}^{3}\to\mathbb{R}$ is the uniform electric potential applied globally on ASSB.
		\end{mdframed}
		\begin{minipage}{0.5\textwidth}
			\begin{mdframed}
				\textbf{Strain energy}: 
				Deformation of solid-state electrolyte 
		...
		\begin{minipage}{0.49\textwidth}
			\begin{mdframed}
				...
				Opening crevice 
				at (SE|SSE)-interface
				influenced by prescribed pressure
				caused by dendrite
				\begin{align*} 
