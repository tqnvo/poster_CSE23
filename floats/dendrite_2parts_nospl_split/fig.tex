...
\psfrag{xgr}[c][c] {$\textsc{\scriptsize Grains}$}
\psfrag{xgrb}[c][c] {$\textsc{\scriptsize Grain boundaries}$}
...
\psfrag{ss}[c][c] {$\textsc{\scriptsize Solid-state}$}
\psfrag{sse}[c][c] {$\textsc{\scriptsize electrolyte}$}

\psfrag{masca}[c][c] {$\textsc{\scriptsize Macroscale}$}
\psfrag{mesca}[c][c] {$\textsc{\scriptsize Mesoscale}$}
\psfrag{misca}[c][c] {$\textsc{\scriptsize Microscale}$}

%--------------------------------------------------------

\psfrag{seleb}[c][c] {$\textsc{\scriptsize Misorientation}$}
\psfrag{selec}[c][c] {$\textsc{\scriptsize Multi-preferred directions}$}

\psfrag{dispmu}[c][c] {$\textsc{displacement field } \Bu$}
...
\psfrag{temp}[c][c] {$\textsc{temperature field } \theta$}
\psfrag{te}[c][c] {$\theta$}
...
\includegraphics[width=0.27\textwidth]{dendrite_2parts_nosample_split.eps}
% \caption{Solid-state electrolyte from macroscale to microscale:
% 	Misorientations among grains across boundaries raise issues of not only 
% 	directional development of dendrites, micro-fractures and pulverizations, but also efficiency of 
% 	the charging $C$-rate of batteries.}